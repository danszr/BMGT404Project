...
def names():
	...
	#print(party_member)
	payment_options(party_member, party_number)

def payment_options(payers, payer_number):
	payment_list = []
	#while True:
	...
	payment_amount = 0.0
	for payer_name, amount in payers.items():
				...
				for receiver_name, receiver_amount in payers.items():
					if payers[receiver_name] > 0:
						if math.fabs(payers[giver_name]) == payers[receiver_name]:
							...
							payers.remove(giver_name)
							payers.remove(receiver_name)
						if math.fabs(payers[giver_name]) > payers[receiver_name]:
							owes[giver_name][receiver_name] = payers[receiver_name]
